...
void disastrOS_start(void (*f)(void*), void* f_args, char* logfile){
  ...
  Resource_init();
  Descriptor_init();
  Semaphore_init();         //aggiunte queste due funzioni
  SemDescriptor_init();     //per gestire i semafori e i loro descrittori
  init_pcb=0;
  ...
  syscall_numarg[DSOS_CALL_SEMOPEN]      = 2;                   //cambiato il numero di parametri da passare alla sem_open
